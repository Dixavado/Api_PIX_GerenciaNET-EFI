...
const certificado = fs.readFileSync(process.env.CERTIFICADO_DEV_PATH);
...
const getAccessToken = async () => {
  try {
    const config = {
      method: 'POST',
      url: process.env.DEV_URL + '/oauth/token',
      headers: {
...
router.post('/cob', async (req, res) => {
  try {
    ...
    const { cpf, nome, original, pagador } = req.body;
    ...
    const config = {
      method: 'POST',
      url: process.env.DEV_URL + '/v2/cob',
      headers: {
    ...
    const { status, devedor, valor, solicitacaoPagador, qrcode, imagemQrcode } = response.data;
    ...
    const qrcodeConfig = {
      method: 'GET',
      url: `${process.env.DEV_URL}/v2/loc/${cobrancaId}/qrcode`,
      headers: {
        Authorization: `${tokenType} ${accessToken}`,
        'Content-Type': 'application/json',
        // Certificado
